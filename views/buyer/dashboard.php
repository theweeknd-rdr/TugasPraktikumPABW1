    ...
    <div class="flex min-h-screen flex-col lg:flex-row">
        <aside class="border-b border-slate-200 bg-white/95 px-5 py-5 lg:sticky lg:top-0 lg:h-screen lg:w-72 lg:border-b-0 lg:border-r">
            ...
            <div class="mt-7 rounded-3xl bg-teal-50 p-4">
                ...
                <p class="mt-1 text-sm text-slate-600">Pantau pesanan dan pilih Open Jastip aktif.</p>
            </div>

            <nav class="mt-7 grid gap-2 text-sm font-bold text-slate-600">
                <a href="<?= Controller::url('/buyer/dashboard') ?>" class="rounded-2xl bg-teal-700 px-4 py-3 text-white hover:no-underline">Dashboard</a>
                ...
                <a href="<?= Controller::url('/buyer/orders/history') ?>" class="rounded-2xl px-4 py-3 hover:bg-slate-100 hover:text-teal-700 hover:no-underline">Riwayat</a>
        ...
        <main class="flex-1 px-5 py-6 sm:px-7 lg:px-10">
            <header class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                <div>
                    <p class="text-sm font-black uppercase tracking-wide text-teal-700">Dashboard Buyer</p>
                    <h1 class="mt-2 text-3xl font-black tracking-normal text-slate-950 sm:text-4xl">Pantau Titipan Kamu</h1>
                    <p class="mt-3 max-w-2xl text-slate-600">Lihat status pesanan, cek proses pengantaran, dan titip barang lewat Open Jastip provider.</p>
                </div>
            ...
            <section class="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <h2 class="mt-3 text-3xl font-black text-slate-950"><?= (int) $totalOrders ?></h2>
                    <p class="mt-3 text-sm text-slate-500">Semua riwayat buyer</p>
                </article>
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm font-bold text-slate-500">Pending</p>
                    <h2 class="mt-3 text-3xl font-black text-amber-700"><?= (int) $pendingOrders ?></h2>
                    <p class="mt-3 text-sm text-slate-500">Menunggu ACC provider</p>
                </article>
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm font-bold text-slate-500">Diproses</p>
                    <h2 class="mt-3 text-3xl font-black text-sky-700"><?= (int) $processedOrders ?></h2>
                    <p class="mt-3 text-sm text-slate-500">Sedang diantar</p>
                </article>
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <p class="text-sm font-bold text-slate-500">Selesai</p>
                    <h2 class="mt-3 text-3xl font-black text-emerald-700"><?= (int) $completedOrders ?></h2>
                    <p class="mt-3 text-sm text-slate-500">Sudah diterima</p>
                </article>
            </section>
            ...
            <section class="mt-7 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
                    <div>
                        <h2 class="text-xl font-black text-slate-950">Pesanan Terbaru</h2>
                        <p class="mt-1 text-sm text-slate-500">Ringkasan pesanan terakhir yang kamu buat.</p>
                    </div>
                    <a href="<?= Controller::url('/buyer/orders/history') ?>" class="rounded-full bg-teal-50 px-4 py-2 text-sm font-black text-teal-700 hover:bg-teal-100 hover:no-underline">Lihat Semua</a>
                </div>

                <?php if (empty($orders)): ?>
                    <div class="mt-5 rounded-3xl bg-slate-50 p-8 text-center">
                        <p class="font-bold text-slate-700">Belum ada pesanan.</p>
                        <p class="mt-2 text-sm text-slate-500">Mulai pesanan pertama kamu dari daftar Open Jastip aktif.</p>
                    </div>
                <?php else: ?>
                    <div class="mt-5 grid gap-4 lg:grid-cols-2">
                        <?php foreach ($orders as $order): ?>
                            <?php $status = (string) ($order['status'] ?? 'pending'); ?>
                            <article class="rounded-3xl border border-slate-200 p-5 transition hover:border-teal-100 hover:shadow-lg">
                                <div class="flex items-start justify-between gap-4">
                                    <div>
                                        <h3 class="text-lg font-black text-slate-950"><?= htmlspecialchars((string) ($order['nama_barang'] ?? 'Barang tidak tersedia'), ENT_QUOTES, 'UTF-8') ?></h3>
